...
void LCD::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) 
{
   ...
   // 50
   // ---------------------------------------------------------------------------
   delayMicroseconds(50000); // 100ms delay
      
   // 4-Bit initialization sequence from Technobly
   send(0x03, FOUR_BITS);         // Put back into 8-bit mode
   delayMicroseconds(5000);

   send(0x08, FOUR_BITS);         // Comment this out for V1 OLED
   delayMicroseconds(5000);       // Comment this out for V1 OLED

   send(0x02, FOUR_BITS);         // Put into 4-bit mode
   delayMicroseconds(5000);
   send(0x02, FOUR_BITS);
   delayMicroseconds(5000);
   send(0x08, FOUR_BITS);
